...
```bash
curl -fsS "$CAREERPILOT_API/api/email/account"
```
...
```bash
curl -fsS "$CAREERPILOT_API/api/email/messages/<id>"
```

**All pending** — no argument. Sync, then pull the unscanned queue:

```bash
curl -fsS -X POST "$CAREERPILOT_API/api/email/sync"
curl -fsS "$CAREERPILOT_API/api/email/messages?reviewStatus=pending&classification=null"
```
...
```bash
curl -fsS -X PATCH "$CAREERPILOT_API/api/email/messages/<id>" \
