...
Follow `../shared/setup.md`. Fetch the campaign: `curl -fsS "$CAREERPILOT_API/api/campaigns/<campaign-id>"`. Threshold = `config.minScore` (fallback `data.autoApply.minMatchScore`, else 70).
...
```bash
curl -fsS -X PATCH "$CAREERPILOT_API/api/campaigns/<campaign-id>/jobs/<key>" \
