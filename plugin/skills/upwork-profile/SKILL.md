...
Follow `../shared/setup.md`. `Read` the resume at `data.primaryResumeSourceAbsolutePath` for identity, summary, experience, skills, and **projects** (the portfolio source). `$CAREERPILOT_API=http://localhost:8000`. Log in to Upwork via `../shared/auth.md` (resolve via `/api/credentials/resolve?domain=upwork.com`).
...
3. **Save the draft** for review:
   ...
   ```bash
   curl -fsS -X PUT "$CAREERPILOT_API/api/upwork/profile" -H 'content-type: application/json' \
...
1. `curl -fsS "$CAREERPILOT_API/api/upwork/profile"` → require `data.status == "approved"`. If not, tell the user to review and approve on `/upwork/profile` first, then stop.
