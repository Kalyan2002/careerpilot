...
```bash
curl -fsS "$CAREERPILOT_API/api/resumes/$BASE_ID"
```
...
```bash
curl -fsS "$CAREERPILOT_API/api/resumes/$BASE_ID/variants"
```
...
```bash
curl -fsS -X POST "$CAREERPILOT_API/api/resumes/$BASE_ID/tailor" \
