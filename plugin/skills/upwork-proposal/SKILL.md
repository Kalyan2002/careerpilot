...
The argument is either a **proposal id** (an integer, when launched from the CareerPilot UI) or a raw **job description** (manual use). Detect which:

- **Integer id** → fetch the draft row and use its stored job details as the JD:

  ```bash
  curl -fsS "$CAREERPILOT_API/api/upwork/proposals/$ARG"
  ```
...
- **Launched with an id** → `PATCH` the existing draft (status stays `draft`):
  ...
  ```bash
  curl -fsS -X PATCH "$CAREERPILOT_API/api/upwork/proposals/$ARG" \
...
- **Launched with a raw job description** → `POST` a new row:
  ...
  ```bash
  curl -fsS -X POST "$CAREERPILOT_API/api/upwork/proposals" \
    -H 'content-type: application/json' \
