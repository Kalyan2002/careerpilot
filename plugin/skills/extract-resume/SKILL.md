...
Read a resume's uploaded source PDF and produce JSON matching the CareerPilot resume schema, then save via the API. Inverse of the editor.
...
```bash
curl -fsS "$CAREERPILOT_API/api/resumes/$RESUME_ID"
```
...
- Otherwise → `${CAREERPILOT_WORKSPACE_ROOT}/src/web/storage/resumes/{sourceFilename}`.
...
```bash
curl -fsS -X PUT "$CAREERPILOT_API/api/resumes/$RESUME_ID" \
